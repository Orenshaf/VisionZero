...
export default function FetchData() {
    setTimeout(TotalData(manhattanUrl).then(response => {
        ParseData(response).then((res) => {
            ...
            ChooseData(Store["MANHATTAN"], ["MANHATTAN"], map, response);
        ...
        TotalData(queensUrl).then(response => {
            ParseData(response).then(res => {
                Store["QUEENS"] = res;
                ChooseData(Store["QUEENS"], "QUEENS", map, response);
            })
        }).then(() => {
            TotalData(brooklynUrl).then(response => {
                ParseData(response).then(res => {
                    Store["BROOKLYN"] = res;
                    ChooseData(Store["BROOKLYN"], "BROOKLYN", map, response);
                })
            }).then(() => {
                TotalData(bronxUrl).then(response => {
                    ParseData(response).then(res => {
                        Store["BRONX"] = res;
                        ChooseData(Store["BRONX"], "BRONX", map, response);
                    })
                }).then(() => {
                    TotalData(statenIslandUrl).then(response => {
                        ParseData(response).then(res => {
                            Store["STATEN ISLAND"] = res;
                            ChooseData(Store["STATEN ISLAND"], "STATEN ISLAND", map, response);
                        })
                    })
                })
            })
